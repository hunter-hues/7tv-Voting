...
async def update_voting_event(event_id: int, update_data: VoteEventUpdate, request: Request, db: AsyncSession = Depends(get_database)):
    ...
    if update_data.time_tab == "endTime" and update_data.end_time:
        # Validate
        min_time = event.created_at + timedelta(minutes=5)
        max_time = event.created_at + timedelta(days=31)
        if update_data.end_time < min_time or update_data.end_time > max_time:
            return {"success": False, "message": "End time must be 5min-31days from creation"}
        # Update
        ...
        new_end_time = datetime.now(dt_timezone.utc) + timedelta(hours=update_data.duration_hours)
        # Validate
        min_time = datetime.now(dt_timezone.utc) + timedelta(minutes=5)
        max_time = event.created_at + timedelta(days=31)
        if new_end_time < min_time or new_end_time > max_time:
            return {"success": False, "message": "Duration must be 5min-31days from creation"}
...
async def get_voting_events(request: Request, db: AsyncSession = Depends(get_database)):
    ...
    for row in allowed_events:
        ...
        event_data = {
            "id": event.id,
            "title": event.title,
            "creator_username": owner_username or "Unknown",
